...
localgeist uses a **CORS Proxy** to handle Cross-Origin Resource Sharing restrictions for:
...
- **`src/tools/extract-document.ts`**:
  - Tool has optional `corsProxyUrl` property set by localgeist if proxy is enabled
...
- **`src/agent/transports/proxy-types.ts`**:
  ...
  - Not used by localgeist - only `ProviderTransport` is used
...
- **`src/agent/transports/AppTransport.ts`**:
  ...
  - **Not used by localgeist** - localgeist only uses `ProviderTransport`
...
Settings stored in IndexedDB under `localgeist-storage` database:
...
- `src/agent/transports/ProviderTransport.ts` - LLM request proxy logic (USED by localgeist)
- `src/agent/transports/AppTransport.ts` - Alternative transport (NOT used by localgeist)
- `src/agent/transports/proxy-types.ts` - Event types for AppTransport (NOT used by localgeist)
